...
void POTHelper_updateForOutputsFromTILNumericEffects(MinimalState & theState, const vector<double> & minTimestamps, const ActionSegment & act,
    ...
    for (; neItr != neEnd; ++neItr) {        
        ...
        if (NumericAnalysis::getDominanceConstraints()[neItr->first] == NumericAnalysis::E_IRRELEVANT) {
            continue;
        }
...
void registerFluentRelated(MinimalState & theState, RPGBuilder::ArtificialVariable & currAV, int startStepID, int  endStepID, const bool & registerNotDeregister){
    bool localDebug = false;
    static const int PNEcount = RPGBuilder::getPNECount();
    for (int i = 0; i< currAV.size; ++i){
	const int lv = currAV.fluents[i];
	...
	for (int j =0; j<currAV.size; ++j){
	    const int lu = currAV.fluents[j];
	    int idj = findIDVariable(lu);
	    if (idj!=id){
    ...
    for (; precItr != precEnd; ++precItr) {
        ...
        for (int pass = 0; pass < 2; ++pass) {
            ...
            if (currVar < PNEcount) {
                ...
                RPGBuilder::ArtificialVariable & currAV = RPGBuilder::getArtificialVariable(currVar);
		registerFluentRelated(theState,currAV,endStepID,startStepID,registerNotDeregister);
                for (int i = 0; i < currAV.size; ++i) {
                    const int lv = currAV.fluents[i];
                    if (lv < PNEcount) {
			mentioned.insert(lv);
